...
| **Source de données** | Liste des sources de données (**nom de la source de données**) utilisées pour enrichir les données des parcours, par exemple Experience Platform, système de réservation. |
...
| **Taux d’éléments en blackliste** | Nombre total de messages marqués comme placés en blackliste par rapport aux messages envoyés. |
